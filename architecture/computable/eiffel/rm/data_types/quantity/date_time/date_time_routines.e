...
feature -- Conversion
	...
	date_to_iso8601_string (a_date: DATE): STRING is
		...
		do
			...
			Result_valid: Result /= Void and then is_valid_iso8601_date_string(Result)				
		end
		
	iso8601_string_to_date (str: STRING): DATE is
			-- make from string using ISO8601 format "yyyy-MM-dd"
		require
			str_valid: str /= Void and then is_valid_iso8601_date_string(str)
		local
			y, m, d: INTEGER
		do
	...
	time_to_iso8601_string (a_time: TIME): STRING is
		...
		do
			...
			Result_valid: Result /= Void and then is_valid_iso8601_time_string(Result)		
		end
		
	iso8601_string_to_time (str: STRING): TIME is
			-- make from string using ISO8601 format "hh:mm:ss[.ssss]"
		require
			str_valid: str /= Void and then is_valid_iso8601_time_string(str)
		local
			h, m, s: INTEGER
			fs: DOUBLE
			sec_str: STRING
	...
	date_time_to_iso8601_string (a_dt: DATE_TIME): STRING is
		...
		do
			...
			Result_valid: Result /= Void and then is_valid_iso8601_date_time_string(Result)
		end
		
	iso8601_string_to_date_time (str: STRING): DATE_TIME is
			-- make from string using ISO8601 format "yyyy-MM-dd hh:mm:ss[.ssss]"
		require
			str_valid: str /= Void and then is_valid_iso8601_date_time_string(str)
		local
			dt: DATE
			tm: TIME
			spc_pos: INTEGER
	...
	duration_to_iso8601_string (a_dur: DATE_TIME_DURATION): STRING is
		...
		do
			...
			Result_valid: Result /= Void and then is_valid_iso8601_duration_string(Result)
		end
		
	iso8601_string_to_duration (str: STRING): DATE_TIME_DURATION is
			-- make from string using ISO8601 format "PNNdNNhNNmNNs"
		require
			str_valid: str /= Void and then is_valid_iso8601_duration_string(str)
		local
			str1: STRING
			d,h,m,s:INTEGER
			left, right: INTEGER
...
feature -- Status Report
	...
	is_valid_iso8601_time_string(s: STRING): BOOLEAN is
	...
	is_valid_iso8601_date_string(s: STRING): BOOLEAN is
	...
	is_valid_iso8601_date_time_string(s: STRING): BOOLEAN is
	...
	is_valid_iso8601_duration_string(s: STRING): BOOLEAN is
